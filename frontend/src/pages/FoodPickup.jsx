...
import { FaMotorcycle, FaEye, FaSearch } from 'react-icons/fa';
import '../style/foodpickup.css';
...
const FoodPickup = () => {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const res  = await fetch('http://127.0.0.1:1000/api/foodpickup/');
      ...
      <div className="foodpickup-page">
        ...
        <div className="orders-header">
          <div className="header-left">
            ...
            <div>
              ...
              <p>Orders currently on the way to customers</p>
            </div>
          </div>
          <div className="header-stats">
            ...
            <div className="stat-card stat-total">
              ...
              <span className="stat-label">Total Pickup</span>
            </div>
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="empty-orders">
            ...
            <p>There are no orders out for delivery right now</p>
          </div>
        ) : (
          <div className="table-card">
            <div className="table-responsive">
              <table className="orders-table">
                <thead>
                  <tr>
                    {['Order #', 'Customer', 'Address', 'Items', 'Total', 'Payment', 'Date', 'Action'].map(h => (
                      <th key={h}>{h}</th>
                    ))}
                  </tr>
                </thead>
            ...
            <div className="table-footer">
              <span>Showing <strong>{filtered.length}</strong> of <strong>{orders.length}</strong> pickup orders</span>
